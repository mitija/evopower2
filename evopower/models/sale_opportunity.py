...
class SaleOpportunity(models.Model):
    ...
    chance_of_sale = fields.Float(
        string="Chance of Sale", compute="_compute_chance_of_sale"
    )
    ...
    weighted_nominal = fields.Integer(
        string="Weighted Nominal", compute="_nominal_value"
    )
    ...
    weighted_forecast = fields.Integer(
        string="Weighted Forecast", compute="_weighted_forecast"
    )

    power_kW_opportunity = fields.Integer(
        string="Power kW Opportunity", compute="_power_kW_opportunity"
    )
    usable_kWh_opportunity = fields.Integer(
        string="Usable kWh Opportunity", compute="_usable_kWh_opportunity"
    )

    kw_weighted_forecast = fields.Integer(
        string="kW Weighted Forecast", compute="_kw_weighted_forecast"
    )
    kwh_weighted_forecast = fields.Integer(
        string="kWh Weighted Forecast", compute="_kwh_weighted_forecast"
    )

    lost_reason_name = fields.Char(
        related="lost_reason_id.display_name", string="Lost Reason Name"
    )
    lost_feedback = fields.Html("Lost Reason Detail", sanitize=True)
    ...
    def _kw_weighted_forecast(self):
        for record in self:
            record.kw_weighted_forecast = (
                record.power_kW_opportunity * record.chance_of_sale
            )

    @api.depends("product_ids", "probability")
    def _kwh_weighted_forecast(self):
        for record in self:
            record.kwh_weighted_forecast = (
                record.usable_kWh_opportunity * record.chance_of_sale
            )
...
class EvoCrmProduct(models.Model):
    ...
    company_currency = fields.Many2one(
        related="lead_id.company_currency", readonly=True, store=True
    )
    ...
    product_price = fields.Float(
        related="product_ids.list_price", string="Price", readonly=False, store=True
    )
    subtotal = fields.Integer(_("Subtotal"), compute="_count_subtotal")
    product_subtotal = fields.Integer(
        _("Product Subtotal"), compute="_count_subtotal_kw"
    )
    power_kW_opportunity = fields.Integer(
        string="Power kW Opportunity", compute="_power_kW_opportunity"
    )
    ...
    def _count_subtotal_kw(self):
        for product in self:
            product.product_subtotal = (
                product.power_kW_opportunity + product.usable_kWh_opportunity
            )
